...
with tab1:
    ...
    st.caption(f"[DEBUG] device_power: {len(df_recent_debug)}行 / device_power_30min: {len(df_agg_debug)}行, 最新={df_agg_debug['recorded_at'].max() if not df_agg_debug.empty else 'なし'}")
